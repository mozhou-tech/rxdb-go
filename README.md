...
- ✅ **LightRAG**：基于 rxdb-go 实现的 RAG 框架，支持混合搜索和 LLM 集成
- ✅ **加密**：支持字段级和数据库级（存储层）加密
- ✅ **关联文档**：支持 `Populate()` 方法加载关联文档
- ✅ **附件**：支持文档附件存储（支持加密）
- ✅ **冲突处理**：可配置的冲突解决策略
...
- `Data()` - 获取文档数据
- `Get(field)` - 获取字段值
- `Set(ctx, field, value)` - 设置字段值
- `Update(ctx, updates)` - 更新并保存文档
- `Save(ctx)` - 保存当前变更
- `Remove(ctx)` - 删除文档
- `Populate(ctx, field)` - 加载关联文档
- `Resync(ctx)` - 重新同步文档
- `Synced(ctx)` - 返回同步状态通道
